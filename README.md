...
If startup fails with `column "id" referenced in foreign key constraint does not exist`, your local DB likely has a legacy `users` table schema. In non-production mode, app startup now auto-repairs by dropping legacy `users/documents/document_chunks` and recreating the expected schema.
